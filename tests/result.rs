...
fn u128_be() {
    ...
    assert!(matches!(
        ...
        Ok(1339673755198158349044581307228491536)
    ));
    assert!(matches!(
        data.read_u128::<BigEndian>(),
...
fn uint_be() {
    ...
    assert!(matches!(
        ...
        Ok(18591708106338011145)
    ));
    assert!(matches!(
        data.read_uint128::<BigEndian>(10),
        Ok(4759477275222530853130)
    ));
    assert!(matches!(
        data.read_uint128::<BigEndian>(11),
        Ok(1218426182456967898401291)
    ));
    assert!(matches!(
        data.read_uint128::<BigEndian>(12),
        Ok(311917102708983781990730508)
    ));
    assert!(matches!(
        data.read_uint128::<BigEndian>(13),
        Ok(79850778293499848189627010061)
    ));
    assert!(matches!(
        data.read_uint128::<BigEndian>(14),
        Ok(20441799243135961136544514575630)
    ));
    assert!(matches!(
        data.read_uint128::<BigEndian>(15),
        Ok(5233100606242806050955395731361295)
    ));
    assert!(matches!(
...
fn int_be() {
    ...
    assert!(matches!(
        ...
        Ok(-35603820666583417)
    ));
    assert!(matches!(
        data.read_int128::<BigEndian>(8),
        Ok(-9114578090645354616)
    ));
    assert!(matches!(
        data.read_int128::<BigEndian>(9),
        Ok(-2333331991205210781559)
    ));
    assert!(matches!(
        data.read_int128::<BigEndian>(10),
        Ok(-597332989748533960078966)
    ));
    assert!(matches!(
        data.read_int128::<BigEndian>(11),
        Ok(-152917245375624693780215157)
    ));
    assert!(matches!(
        data.read_int128::<BigEndian>(12),
        Ok(-39146814816159921607735080052)
    ));
    assert!(matches!(
        data.read_int128::<BigEndian>(13),
        Ok(-10021584592936939931580180493171)
    ));
    assert!(matches!(
        data.read_int128::<BigEndian>(14),
        Ok(-2565525655791856622484526206251634)
    ));
    assert!(matches!(
        data.read_int128::<BigEndian>(15),
        Ok(-656774567882715295356038708800418161)
    ));
    assert!(matches!(
        data.read_int128::<BigEndian>(16),
        Ok(-168134289377975115611145909452907049192)
    ));
